 ...
 * them. The scoresheet, the check mark and the slide are the same deal: the
 * server sends the notation, the square of the checked king and the move in
 * UCI, and none of the three is worked out again here.
 */
export function ChessBoardView({
  ...
  // The three `?? []`s are not belt-and-braces on the types, which say these
  // are always arrays. They are the version skew: the desktop auto-updates and
  // the backend is deployed separately, so a build carrying this file can meet
  // a server that predates `history`. Iterating an undefined inside a useMemo
  // is a throw during render, which the error boundary turns into a dead page --
  // exactly how the null `legalMoves` of a mated board surfaced.
  const pieces = useMemo(() => parseFenPieces(board.fen), [board.fen]);
  const moves = useMemo(
    () => movesByOrigin(board.legalMoves ?? []),
    [board.legalMoves],
  );
  ...
  // Where the arriving piece has to start from, so it can slide home. null on
  // a fresh board and on a move whose squares did not parse.
  ...
  return (
    <div className="ct-chess">
      <MoveTicker board={board} mySeat={mySeat} />

      <div className="ct-chess-layout">
        <div
        ...
        >
          {order.map((index, position) => {
            ...
            // The piece that just landed, and only that one — keying every
            // glyph off the move would remount the whole board on every turn.
            ...
            return (
              <button
                ...
                // The server names the king in danger; this file does not work
                // out that anybody is in check.
              ...
              >
                {piece ? (
                  <span
                    ...
                    // whose key did not change, and a reused node does not
                    // restart its animation. Only the arriving piece takes a
                    // per-move key, so nothing else remounts.
                ...
                {/* Coordinates on the two outer edges, as on a real board. The
                    scoresheet next to it is written in them, so without these
                    "Nf3" is a word rather than a square. */}
                {position % 8 === 0 ? (
        ...
        <MoveList pairs={pairs} plies={history.length} />
      </div>
 ...
 * nothing told you the turn had come round. It is announced to screen readers
 * as well, which is the same information by another route rather than an extra
 * feature.
 */
function MoveTicker({ board, mySeat }: { board: MinigameChessBoard; mySeat: number }) {
  ...
  return (
    <div
      ...
      // Whose move it announces, so the dot next to it wears that side's colour
    ...
    >
      ...
      {/* Only the server sets this, and only for the king that is actually in
          danger. `outcome` is non-empty exactly when the game is over, which
          on a checked king can only mean mate. */}
      {board.checkSquare ? (
 ...
 * The scoresheet.
 *
 * Kept next to the board rather than under it so a glance covers both, and
 * scrolled to the bottom on every move — the interesting end of a move list is
 * always the new end, and a list that has to be dragged is a list nobody reads.
 */
function MoveList({ pairs, plies }: { pairs: readonly MovePair[]; plies: number }) {
